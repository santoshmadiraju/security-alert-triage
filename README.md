...
(Project 2). This project fine-tunes a **pretrained** Hugging Face model
(DistilBERT) on a **labeled** classification task, predicting whether a log
line is a real alert or normal noise, framed as the kind of triage a SOC
...
precision barely moved (0.9999 to 0.9978), but recall dropped from 97% to
...
- **Data:** [BGL](https://github.com/logpai/loghub) (Blue Gene/L
  supercomputer logs, loghub family). 4,747,963 lines, 348,460 labeled
  alerts (7.34%). Split chronologically 70/15/15 by line order (train on
  the earliest data, test on the latest).
- **Input:** only the free-text log message, masked (`<NUM>`, `<HEX>`,
  ...
  `level in {FATAL, FAILURE}`, but that alone is a weak signal (FATAL-level
...
- **Training:** 2 of 3 planned epochs completed (see Hardware note below).
  Epoch 1 to epoch 2 validation F1 was 0.986 to 0.986, already converged, so
...
throttling (GPU SM clock pinned at 255MHz of a possible 2100MHz at 89C)
...
| File | Purpose |
|---|---|
| `download_data.py` | fetch and unpack BGL from loghub/Zenodo |
